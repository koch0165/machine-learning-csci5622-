...
class Knearest:
    ...
    def majority(self, item_indices):
        ...
        labelcountArray = Counter(labels)

        majorlabelTuple = labelcountArray.most_common(1)
        majorlabelCount = majorlabelTuple[0][1]

        majorlabels = []

        for label in labelcountArray:
            if labelcountArray[label] == majorlabelCount:
                majorlabels.append(label)

        majorlabels.sort()
        return median(majorlabels)
    ...
    def classify(self, example):
        ...
        """
        example = numpy.reshape(example, (-1, example.size))
    ...
    def confusion_matrix(self, test_x, test_y, debug=False):
        ...
        for xx, yy in zip(test_x, test_y):
            classifiedlabel = self.classify(xx)
         #   print("actual label %d classified label %d data_index: %d " % (yy, classifiedlabel, data_index))
            if yy in d and classifiedlabel in d[yy]:
                    d[yy][classifiedlabel] = d[yy][classifiedlabel] + 1
            else :
                    d[yy][classifiedlabel] = 1
...
if __name__ == "__main__":
    ...
    print("train data size : %d " %data.train_y.size)
    if args.limit > 0:
    ...
    print("Test data size : %d " %data.test_y.size )
